...
update_script() {
    ...
    new_version=$(cat /tmp/nezha.sh | grep "VERSION" | head -n 1 | awk -F "=" '{print $2}' | sed 's/\"//g;s/,//g;s/ //g')
...
install_agent() {
    ...
    echo -e "${green}> 安装监控Agent指定版本${VERSION}${plain}"
...
show_usage() {
    ...
    echo "./nezha.sh restart_and_update         - 重启并降级面板"
